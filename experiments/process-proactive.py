...
def processResults(rootdir):
  ...
  for p in result_folder_p:
      ...
      with open (file, 'r') as f:
        for line in f:
          if line and "Sending new" in line:
            a = line.split()
            prefix = "/"+'/'.join(a[7].split("/")[4:])
            timestamp = a[0]
            # if (p == 'p1'):
  ...
  for c in result_folder_c:
    ...
    with open (file, 'r') as f:
      for line in f:
        if line and "Received data for interest:" in line:
          a = line.split()
          prefix = a[7]
          timestamp = a[0]
          result_dict[c][a[7]] = a[0]
          try:
  ...
  with open ("2-loss-proactive.csv", 'w') as f:
    write = csv.writer(f)
    write.writerows(final_result)

if __name__ == '__main__':
    processResults("/home/mini-ndn/europa_bkp/mini-ndn/sdulal/mini-ndn/ndn-src/NDNSD/experiments/result/comparision/p_interval_10/proactive/loss_2")
